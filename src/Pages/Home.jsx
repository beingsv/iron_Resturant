...
const Home = () => {
  ...
  const fetchData = async (lat, lon) => {
    ...
    const resturant = json.data.cards[2].card.card.gridElements.infoWithStyle.restaurants;
      ...
      <>
        ...
        <div className='pt-4 px-8 flex justify-between'>
          <button
            ...
            onClick={() => {
              const filteredRestaurants = listRestaurants.filter((res) => res.info.avgRating > 4.1);
          ...
          <div className='flex gap-2'>
            ...
            <button
              ...
              onClick={() => {
                const searchedCard = listRestaurants.filter((res) => res.info.name.toLowerCase().includes(searchedText.toLowerCase()));
        ...
        <div className='flex flex-wrap justify-center'>
          {searchedRestaurant.map((res) => {
            return (
              <div key={res.info.id} className='w-[200px] overflow-hidden border rounded-lg m-5 hover:transition duration-300 hover:scale-105'>
                <Link key={res.info.id} to={`/restaurant/${res.info.id}`}><ResCard res={res} /></Link>
              </div>
            )
          })}
        </div>
